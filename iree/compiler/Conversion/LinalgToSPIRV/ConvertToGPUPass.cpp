...
struct ConvertToGPUPass : public PassWrapper<ConvertToGPUPass, FunctionPass> {
  ...
  Option<bool> legacyConvLowering{
      *this, "legacy-conv-lowering",
...
/// non-zero entries.
template <typename LinalgOpTy>
static bool hasPadding(LinalgOpTy op) {
...
// TODO(#2134) : The default path of using `MapLinalgOpToLocalInvocationId`
// seems to have a bug. It only shows up currently on Resnet50. Leaving the
// legacy path till it can be triaged.
template <typename LinalgOpTy>
struct MapConvPoolToLocalInvocationId : public OpConversionPattern<LinalgOpTy> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (hasPadding(linalgOp))
      pLoopOp =
          cast<scf::ParallelOp>(serializeDimensionsFrom(rewriter, pLoopOp, 1));
...
#undef ADD_ALL_LINALG_PATTERNS
    ...
    MapConvPoolToLocalInvocationId<linalg::PoolingMaxOp>,
    MapConvPoolToLocalInvocationId<linalg::PoolingMinOp>,
    MapConvPoolToLocalInvocationId<linalg::PoolingSumOp>,
    MapLinalgOpToLocalInvocationId<linalg::MatmulOp>,
    PartitionPLoopToWorkgroups, RemoveLinalgRange>(context);
  // clang-format on

  if (legacyConvLowering)
