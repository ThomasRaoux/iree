...
module attributes {spv.target_env = #spv.target_env<#spv.vce<v1.3, [Shader], [SPV_KHR_storage_buffer_storage_class]>, {max_compute_workgroup_invocations = 128 : i32, max_compute_workgroup_size = dense<[128, 128, 64]> : vector<3xi32>}>} {
  func @conv_padding(%arg0: memref<?x?x?x?xf32>, %arg1: memref<?x?x?x?xf32>, %arg2: memref<?x?x?x?xf32>) attributes {spv.entry_point_abi = {local_size = dense<[32, 1, 1]> : vector<3xi32>}} {
    ...
    %c1 = constant 1 : index
    %0 = dim %arg1, %c0 : memref<?x?x?x?xf32>
    scf.parallel (%arg3) = (%c0) to (%0) step (%c32) {
      %1 = dim %arg1, %c0 : memref<?x?x?x?xf32>
      %2 = affine.min #map0(%c32, %1, %arg3)
      %3 = dim %arg1, %c1 : memref<?x?x?x?xf32>
      %4 = dim %arg1, %c2 : memref<?x?x?x?xf32>
      %5 = dim %arg1, %c3 : memref<?x?x?x?xf32>
      %6 = subview %arg1[%arg3, %c0, %c0, %c0] [%2, %3, %4, %5] [%c1, %c1, %c1, %c1]  : memref<?x?x?x?xf32> to memref<?x?x?x?xf32, #map1>
      %7 = dim %arg2, %c0 : memref<?x?x?x?xf32>
      %8 = affine.min #map0(%c32, %7, %arg3)
      %9 = dim %arg2, %c1 : memref<?x?x?x?xf32>
      %10 = dim %arg2, %c2 : memref<?x?x?x?xf32>
      %11 = dim %arg2, %c3 : memref<?x?x?x?xf32>
      %12 = subview %arg2[%arg3, %c0, %c0, %c0] [%8, %9, %10, %11] [%c1, %c1, %c1, %c1]  : memref<?x?x?x?xf32> to memref<?x?x?x?xf32, #map1>
      linalg.conv(%arg0, %6, %12) {__internal_linalg_transform__ = "workitem", dilations = [1, 1], padding = dense<[[1, 1], [0, 1]]> : tensor<2x2xi64>, strides = [1, 1]} : memref<?x?x?x?xf32>, memref<?x?x?x?xf32, #map1>, memref<?x?x?x?xf32, #map1>
      scf.yield
    }
    return
  }
...
//   CHECK-DAG:   %[[BOFFSETX:.+]] = muli %[[BIDX]], %[[C32]]
//   CHECK-DAG:   %[[BSTEPX:.+]] = muli %[[NBLOCKSX]], %[[C32]]
//       CHECK:   scf.for %[[IV3:.+]] = %[[BOFFSETX]] to %[[UB0]] step %[[BSTEPX]]
//       CHECK:     %[[BOUNDSX:.+]] = affine.min #{{.*}}(%[[IV3]])
//       CHECK:     %[[SV1:.+]] = subview %[[ARG1]][%[[IV3]], 0, 0, 0]
//       CHECK:     %[[SV2:.+]] = subview %[[ARG2]][%[[IV3]], 0, 0, 0]
//   CHECK-DAG:     %[[TIDX:.+]] = "gpu.thread_id"() {dimension = "x"}
//   CHECK-DAG:     %[[NTHREADSX:.+]] = "gpu.block_dim"() {dimension = "x"}
//       CHECK:     scf.for %[[IV4:.+]] = %[[TIDX]] to %[[BOUNDSX]] step %[[NTHREADSX]]
//       CHECK:       scf.for
//       CHECK:         scf.for
//       CHECK:           scf.for
//       CHECK:             scf.for
