...
static bool hasPadding(OpTy op) {
  ...
  return llvm::any_of(padding.getValue(),
                      [](APInt v) -> bool { return v.getZExtValue() != 0; });
}
...
LogicalResult TileSizeCalculator::setTileSizesBasedOnOps(
  ...
  // TODO(#2134): With padding only the batch dimension can be tiled. Eventually
  // the padding attribute should be dropped, and pad should be separated from
  // conv and fused later. Alternatively, ops with padding should just be
  // distributed across threads without using tiling. Using the former for now,
  // but keeping the fallback mechanism of tiling batch dimension if needed for
  // traiging/coverage.
...
#define CHECK_CONV_POOL(LinalgOpTy, EnumValue)             \
  if (auto convPoolOp = dyn_cast<LinalgOpTy>(op)) {        \
    opInfo |= EnumValue;                                   \
    hasNonZeroPadding |= hasPadding(cast<LinalgOpTy>(op)); \
  }

    CHECK_CONV_POOL(linalg::ConvOp, OpInfo::Convolution)
    CHECK_CONV_POOL(linalg::PoolingMaxOp, OpInfo::Pooling)
    CHECK_CONV_POOL(linalg::PoolingMinOp, OpInfo::Pooling)
    CHECK_CONV_POOL(linalg::PoolingSumOp, OpInfo::Pooling)

#undef CHECK_CONV_POOL
  }
  ...
  if (opInfo & OpInfo::Convolution) {
    if (hasNonZeroPadding) {
      ...
      // TODO(ravishankarm): This tiling is meant to enable promotion to
      // workgroup memory, but doesnt actually get us to a state where we can do
      // this. The promotion is possible only when the subviews created are
      // constant size. For now this doesnt really matter. Revisit this later.
...
struct TileConvPoolPattern : public TilingPattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // TODO(#2134): Keeping this assert commented out for now for triaging
    // failure in Resnet50 that required use of the fallback.

    // assert(!hasPadding(cast<OpTy>(op)));
