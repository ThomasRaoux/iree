...
static iree_status_t iree_hal_cuda_create_shader_module(
...
static void iree_hal_cuda_destroy_shader_module(
...
static iree_status_t iree_hal_cuda_create_pipelines(
...
static void iree_hal_cuda_destroy_pipeline(VkDeviceHandle* logical_device,
                                             VkPipeline handle) {
  if (handle == VK_NULL_HANDLE) return;
...
iree_status_t iree_hal_cuda_native_executable_create(
  ...
  *out_executable = NULL;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_create_shader_module(
              logical_device,
...
static void iree_hal_cuda_native_executable_destroy(
  ...
  for (iree_host_size_t i = 0; i < executable->pipeline_count; ++i) {
    iree_hal_cuda_destroy_pipeline(executable->logical_device,
...
iree_status_t iree_hal_cuda_native_executable_pipeline_for_entry_point(
