...
static bool iree_hal_cuda_layer_list_contains(uint32_t layer_count,
...
static iree_status_t iree_hal_cuda_match_available_layers(
    iree_host_size_t available_layers_count,
    const VkLayerProperties* available_layers,
    const iree_hal_cuda_string_list_t* required_layers,
    const iree_hal_cuda_string_list_t* optional_layers,
  ...
  for (iree_host_size_t i = 0; i < required_layers->count; ++i) {
    ...
    if (!iree_hal_cuda_layer_list_contains(available_layers_count,
...
iree_status_t iree_hal_cuda_match_available_instance_layers(
    const iree::hal::cuda::DynamicSymbols* syms,
    const iree_hal_cuda_string_list_t* required_layers,
    const iree_hal_cuda_string_list_t* optional_layers, iree::Arena* arena,
    iree_hal_cuda_string_list_t* out_enabled_layers) {
  ...
  out_enabled_layers->values = (const char**)arena->AllocateBytes(
      (required_layers->count + optional_layers->count) *
      sizeof(out_enabled_layers->values[0]));
  return iree_hal_cuda_match_available_layers(
...
static bool iree_hal_cuda_extension_list_contains(
...
static iree_status_t iree_hal_cuda_match_available_extensions(
    iree_host_size_t available_extension_count,
    const VkExtensionProperties* available_extensions,
    const iree_hal_cuda_string_list_t* required_extensions,
    const iree_hal_cuda_string_list_t* optional_extensions,
  ...
  for (iree_host_size_t i = 0; i < required_extensions->count; ++i) {
    ...
    if (!iree_hal_cuda_extension_list_contains(
...
iree_status_t iree_hal_cuda_match_available_instance_extensions(
    const iree::hal::cuda::DynamicSymbols* syms,
    const iree_hal_cuda_string_list_t* required_extensions,
    const iree_hal_cuda_string_list_t* optional_extensions,
    iree::Arena* arena, iree_hal_cuda_string_list_t* out_enabled_extensions) {
  ...
  out_enabled_extensions->values = (const char**)arena->AllocateBytes(
      (required_extensions->count + optional_extensions->count) *
      sizeof(out_enabled_extensions->values[0]));
  return iree_hal_cuda_match_available_extensions(
      extension_property_count, extension_properties, required_extensions,
      optional_extensions, out_enabled_extensions);
}

iree_status_t iree_hal_cuda_match_available_device_extensions(
    const iree::hal::cuda::DynamicSymbols* syms,
...
iree_hal_cuda_instance_extensions_t
iree_hal_cuda_populate_enabled_instance_extensions(
    const iree_hal_cuda_string_list_t* enabled_extensions) {
  iree_hal_cuda_instance_extensions_t extensions;
...
iree_hal_cuda_populate_enabled_device_extensions(
    const iree_hal_cuda_string_list_t* enabled_extensions) {
  iree_hal_cuda_device_extensions_t extensions;
  memset(&extensions, 0, sizeof(extensions));
  for (iree_host_size_t i = 0; i < enabled_extensions->count; ++i) {
    const char* extension_name = enabled_extensions->values[i];
...
iree_hal_cuda_device_extensions_t
iree_hal_cuda_infer_enabled_device_extensions(
    const iree::hal::cuda::DynamicSymbols* device_syms) {
  iree_hal_cuda_device_extensions_t extensions;
  memset(&extensions, 0, sizeof(extensions));
