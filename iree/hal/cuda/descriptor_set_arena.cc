...
PopulateDescriptorSetWriteInfos(
  ...
  for (int i = 0; i < binding_count; ++i) {
    ...
    buffer_info.buffer = iree_hal_cuda_vma_buffer_handle(
    ...
    // "view" to Cuda drivers over the allocated memory.
    //
    ...
    // the shader is considered as out of bounds per the Cuda spec.
...
static VkDescriptorSetAllocateInfo PopulateDescriptorSetsAllocateInfo(
  ...
  VkDescriptorSetLayout set_layout_handle =
      iree_hal_cuda_native_descriptor_set_layout_handle(set_layout);
...
Status DescriptorSetArena::BindDescriptorSet(
  ...
  auto* set_layout =
      iree_hal_cuda_native_executable_layout_set(executable_layout, set);
