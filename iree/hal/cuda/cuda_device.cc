...
#include "iree/hal/cuda/emulated_semaphore.h"
#include "iree/hal/cuda/extensibility_util.h"
#include "iree/hal/cuda/handle_util.h"
#include "iree/hal/cuda/native_descriptor_set.h"
#include "iree/hal/cuda/native_descriptor_set_layout.h"
#include "iree/hal/cuda/native_event.h"
#include "iree/hal/cuda/native_executable_layout.h"
#include "iree/hal/cuda/native_semaphore.h"
#include "iree/hal/cuda/nop_executable_cache.h"
#include "iree/hal/cuda/serializing_command_queue.h"
#include "iree/hal/cuda/status_util.h"
#include "iree/hal/cuda/vma_allocator.h"
...
#define ADD_EXT(target_set, name_literal)                       \
  ...
  ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_DEVICE_EXTENSIONS_REQUIRED,
          VK_KHR_STORAGE_BUFFER_STORAGE_CLASS_EXTENSION_NAME);

  // VK_KHR_get_physical_device_properties2:
  // Multiple extensions depend on VK_KHR_get_physical_device_properties2.
  // This extension was deprecated in Cuda 1.1 as its functionality was
  // promoted to core so we list it as optional even though we require it.
  ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_EXTENSIONS_OPTIONAL,
          VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);

  // VK_KHR_push_descriptor:
  // We can avoid a lot of additional Cuda descriptor set manipulation
  // overhead when this extension is present. Android is a holdout, though, and
  // we have a fallback for when it's not available.
  ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_DEVICE_EXTENSIONS_OPTIONAL,
          VK_KHR_PUSH_DESCRIPTOR_EXTENSION_NAME);

  //===--------------------------------------------------------------------===//
  // Cuda forward-compatibility shims
  //===--------------------------------------------------------------------===//
  ...
  // if timeline semaphores are supported natively (Cuda 1.2+).
  ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_LAYERS_OPTIONAL,
          "VK_LAYER_KHRONOS_timeline_semaphore");
  ...
  // performance behavior of Cuda.

  // VK_LAYER_KHRONOS_validation:
  // only enabled if validation is desired. Since validation in Cuda is just a
  // API correctness check it can't be used as a security mechanism and is fine
  // to ignore.
  if (iree_all_bits_set(requested_features,
                        IREE_HAL_CUDA_FEATURE_ENABLE_VALIDATION_LAYERS)) {
    ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_LAYERS_OPTIONAL,
            "VK_LAYER_KHRONOS_validation");
  }

  // VK_EXT_debug_utils:
  // only enabled if debugging is desired to route Cuda debug messages through
  // our logging sinks. Note that this adds a non-trivial runtime overhead and
  // we may want to disable it even in debug builds.
  if (iree_all_bits_set(requested_features,
                        IREE_HAL_CUDA_FEATURE_ENABLE_DEBUG_UTILS)) {
    ADD_EXT(IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_EXTENSIONS_OPTIONAL,
...
// Returns IREE_HAL_CUDA_INVALID_QUEUE_FAMILY_INDEX if no matching queue is
// found.
static uint32_t iree_hal_cuda_find_first_queue_family_with_flags(
  ...
  return IREE_HAL_CUDA_INVALID_QUEUE_FAMILY_INDEX;
}
...
// available.
static iree_status_t iree_hal_cuda_select_queue_families(
    VkPhysicalDevice physical_device, iree::hal::cuda::DynamicSymbols* syms,
    iree_hal_cuda_queue_family_info_t* out_family_info) {
  ...
  out_family_info->dispatch_index = IREE_HAL_CUDA_INVALID_QUEUE_FAMILY_INDEX;
  out_family_info->dispatch_queue_count = 0;
  out_family_info->transfer_index = IREE_HAL_CUDA_INVALID_QUEUE_FAMILY_INDEX;
...
static iree_status_t iree_hal_cuda_build_queue_sets(
    VkPhysicalDevice physical_device, iree::hal::cuda::DynamicSymbols* syms,
    iree_hal_cuda_queue_set_t* out_compute_queue_set,
    iree_hal_cuda_queue_set_t* out_transfer_queue_set) {
  // Select which queues to use (and fail the implementation can't handle them).
  iree_hal_cuda_queue_family_info_t queue_family_info;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_select_queue_families(
...
static iree_status_t iree_hal_cuda_create_transient_command_pool(
...
static CommandQueue* iree_hal_cuda_device_create_queue(
    VkDeviceHandle* logical_device,
...
// device queue lists.
static void iree_hal_cuda_device_initialize_command_queues(
    iree_hal_cuda_device_t* device, iree_string_view_t queue_prefix,
  ...
  for (iree_host_size_t i = 0; i < compute_queue_count; ++i) {
    ...
    CommandQueue* queue = iree_hal_cuda_device_create_queue(
...
static iree_status_t iree_hal_cuda_device_create_internal(
    ...
    const iree_hal_cuda_device_options_t* options, VkInstance instance,
    VkPhysicalDevice physical_device, VkDeviceHandle* logical_device,
    const iree_hal_cuda_device_extensions_t* device_extensions,
    const iree_hal_cuda_queue_set_t* compute_queue_set,
    const iree_hal_cuda_queue_set_t* transfer_queue_set,
  ...
  iree_status_t status = iree_hal_cuda_vma_allocator_create(
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_create_transient_command_pool(
  ...
  // ony Cuda versions prior to 1.2 when they were made core.
  bool emulate_timeline_semaphores =
      device_syms->vkGetSemaphoreCounterValue == NULL ||
      iree_all_bits_set(
          options->flags,
          IREE_HAL_CUDA_DEVICE_FORCE_TIMELINE_SEMAPHORE_EMULATION);
  ...
  // above.
  if (iree_status_is_ok(status)) {
    iree_hal_cuda_device_initialize_command_queues(
...
iree_status_t iree_hal_cuda_device_create(
    ...
    iree_hal_cuda_syms_t* opaque_syms, VkInstance instance,
  ...
  iree_hal_cuda_string_list_t required_extensions;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_device_query_extensibility_set(
      enabled_features,
      IREE_HAL_CUDA_EXTENSIBILITY_DEVICE_EXTENSIONS_REQUIRED, &arena,
      &required_extensions));
  iree_hal_cuda_string_list_t optional_extensions;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_device_query_extensibility_set(
      enabled_features,
      IREE_HAL_CUDA_EXTENSIBILITY_DEVICE_EXTENSIONS_OPTIONAL, &arena,
      &optional_extensions));
  iree_hal_cuda_string_list_t enabled_extensions;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_match_available_device_extensions(
      instance_syms, physical_device, &required_extensions,
      &optional_extensions, &arena, &enabled_extensions));
  iree_hal_cuda_device_extensions_t enabled_device_extensions =
      iree_hal_cuda_populate_enabled_device_extensions(&enabled_extensions);
  ...
  // are of the same queue family as the dispatch queues: Cuda requires that
  ...
  iree_hal_cuda_queue_set_t compute_queue_set;
  iree_hal_cuda_queue_set_t transfer_queue_set;
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_build_queue_sets(
...
IREE_API_EXPORT iree_status_t IREE_API_CALL iree_hal_cuda_wrap_device(
  ...
  // guaranteed to work.
  iree_hal_cuda_device_extensions_t enabled_device_extensions =
      iree_hal_cuda_infer_enabled_device_extensions(device_syms.get());
  ...
  *logical_device_handle->mutable_value() = logical_device;
  ...
  // Allocate and initialize the device.
  iree_status_t status = iree_hal_cuda_device_create_internal(
...
static iree_status_t iree_hal_cuda_device_create_event(
  ...
  iree_hal_cuda_device_t* device = iree_hal_cuda_device_cast(base_device);
  return iree_hal_cuda_native_event_create(device->logical_device, out_event);
}
...
static iree_status_t iree_hal_cuda_device_create_executable_layout(
  ...
  iree_hal_cuda_device_t* device = iree_hal_cuda_device_cast(base_device);
  return iree_hal_cuda_native_executable_layout_create(
...
static iree_status_t iree_hal_cuda_device_create_semaphore(
  ...
  if (device->semaphore_pool != NULL) {
    return iree_hal_cuda_emulated_semaphore_create(
        device->logical_device, device->semaphore_pool, device->queue_count,
        device->queues, initial_value, out_semaphore);
  }
  return iree_hal_cuda_native_semaphore_create(device->logical_device,
...
static iree_status_t iree_hal_cuda_device_wait_semaphores_with_deadline(
  ...
  if (device->semaphore_pool != NULL) {
    return iree_hal_cuda_emulated_semaphore_multi_wait(
        device->logical_device, semaphore_list, deadline_ns, wait_flags);
  }
  return iree_hal_cuda_native_semaphore_multi_wait(
      device->logical_device, semaphore_list, deadline_ns, wait_flags);
}
...
static iree_status_t iree_hal_cuda_device_wait_semaphores_with_timeout(
  ...
  return iree_hal_cuda_device_wait_semaphores_with_deadline(
