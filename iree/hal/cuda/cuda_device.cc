...
static void iree_hal_cuda_device_destroy(iree_hal_device_t* base_device) {
  ...
  CUDA_CHECK_OK(device->context_wrapper.syms, cuStreamDestroy(device->stream));
...
static iree_status_t iree_hal_cuda_device_create_internal(
    ...
    iree::hal::cuda::DynamicSymbols* syms, iree_allocator_t host_allocator,
...
iree_status_t iree_hal_cuda_device_create(iree_hal_driver_t* driver,
                                          ...
                                          iree_hal_device_t** out_device) {
  CUcontext context;
  CUDA_RETURN_IF_ERROR(syms, cuCtxCreate(&context, 0, device), "cuCtxCreate");
  CUstream stream;
