...
static constexpr const FunctionPtrInfo kInstancelessFunctionPtrInfos[] = {
    IREE_CUDA_DYNAMIC_SYMBOL_INSTANCELESS_TABLE(INS_PFN_FUNCTION_PTR)};
...
static constexpr const FunctionPtrInfo kDynamicFunctionPtrInfos[] = {
    IREE_CUDA_DYNAMIC_SYMBOL_INSTANCE_DEVICE_TABLES(INS_PFN_FUNCTION_PTR,
...
Status ResolveFunctions(DynamicSymbols* syms,
  ...
  // like libcuda.so for their functionality. However, there was an issue
  // with fully treblized Android 10 where /data/local/tmp did not have access
  // to the linker namespaces needed by libcuda.so. This is fixed via
  ...
  // vendor Cuda implementation under /vendor/lib[64]/hw/cuda.*.so as
  // libcuda.so under /data/local/tmp and use LD_LIBRARY_PATH=/data/local/tmp
  // when invoking the test binaries. This effectively bypasses the Android
  // Cuda loader. This is fine for ARM Mali GPUs, whose driver exposes
  // the symbol `vkGetInstanceProcAddr`. But for Qualcomm Adreno GPUs,
  // the Cuda implementation library does not directly expose the symbol.
  ...
  if (!syms->vkGetInstanceProcAddr) {
    return UnavailableErrorBuilder(IREE_LOC)
           << "Required method vkGetInstanceProcAddr not "
              "found in provided Cuda library (did you pick the wrong file?)";
  }
  ...
  for (int i = 0; i < IREE_ARRAYSIZE(kInstancelessFunctionPtrInfos); ++i) {
    ...
    *member_ptr =
    ...
    if (*member_ptr == nullptr) {
      return UnavailableErrorBuilder(IREE_LOC)
             << "Mandatory Cuda function " << function_ptr.function_name
...
Status DynamicSymbols::LoadFromDevice(VkInstance instance, VkDevice device) {
  ...
  if (!this->vkGetDeviceProcAddr) {
    return UnavailableErrorBuilder(IREE_LOC)
           << "Required Cuda function vkGetDeviceProcAddr not available; "
  ...
  for (int i = 0; i < IREE_ARRAYSIZE(kDynamicFunctionPtrInfos); ++i) {
    ...
    if (*member_ptr == nullptr && function_ptr.is_required) {
      return UnavailableErrorBuilder(IREE_LOC)
             << "Required Cuda function " << function_ptr.function_name
             << " not available";
    }
  }
