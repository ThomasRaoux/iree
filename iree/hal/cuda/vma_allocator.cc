...
#include "iree/hal/cuda/vma_buffer.h"

using namespace iree::hal::cuda;

typedef struct iree_hal_cuda_vma_allocator_s {
  iree_hal_resource_t resource;
  iree_allocator_t host_allocator;
  VmaAllocator vma;
} iree_hal_cuda_vma_allocator_t;

extern const iree_hal_allocator_vtable_t iree_hal_cuda_vma_allocator_vtable;

static iree_hal_cuda_vma_allocator_t* iree_hal_cuda_vma_allocator_cast(
    iree_hal_allocator_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_vma_allocator_vtable);
  return (iree_hal_cuda_vma_allocator_t*)base_value;
}

iree_status_t iree_hal_cuda_vma_allocator_create(
  ...
  VmaCudaFunctions cuda_fns;
  memset(&cuda_fns, 0, sizeof(cuda_fns));
  cuda_fns.vkGetPhysicalDeviceProperties =
      syms->vkGetPhysicalDeviceProperties;
  cuda_fns.vkGetPhysicalDeviceMemoryProperties =
      syms->vkGetPhysicalDeviceMemoryProperties;
  cuda_fns.vkAllocateMemory = syms->vkAllocateMemory;
  cuda_fns.vkFreeMemory = syms->vkFreeMemory;
  cuda_fns.vkMapMemory = syms->vkMapMemory;
  cuda_fns.vkUnmapMemory = syms->vkUnmapMemory;
  cuda_fns.vkFlushMappedMemoryRanges = syms->vkFlushMappedMemoryRanges;
  cuda_fns.vkInvalidateMappedMemoryRanges =
      syms->vkInvalidateMappedMemoryRanges;
  cuda_fns.vkBindBufferMemory = syms->vkBindBufferMemory;
  cuda_fns.vkBindImageMemory = syms->vkBindImageMemory;
  cuda_fns.vkGetBufferMemoryRequirements =
      syms->vkGetBufferMemoryRequirements;
  cuda_fns.vkGetImageMemoryRequirements = syms->vkGetImageMemoryRequirements;
  cuda_fns.vkCreateBuffer = syms->vkCreateBuffer;
  cuda_fns.vkDestroyBuffer = syms->vkDestroyBuffer;
  cuda_fns.vkCreateImage = syms->vkCreateImage;
  cuda_fns.vkDestroyImage = syms->vkDestroyImage;
  cuda_fns.vkCmdCopyBuffer = syms->vkCmdCopyBuffer;
  ...
  create_info.pCudaFunctions = &cuda_fns;
  ...
  iree_hal_cuda_vma_allocator_t* allocator = NULL;
  iree_status_t status = iree_allocator_malloc(
      host_allocator, sizeof(*allocator), (void**)&allocator);
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_cuda_vma_allocator_vtable,
                                 &allocator->resource);
...
static void iree_hal_cuda_vma_allocator_destroy(
    iree_hal_allocator_t* base_allocator) {
  iree_hal_cuda_vma_allocator_t* allocator =
      iree_hal_cuda_vma_allocator_cast(base_allocator);
...
static iree_allocator_t iree_hal_cuda_vma_allocator_host_allocator(
    const iree_hal_allocator_t* base_allocator) {
  iree_hal_cuda_vma_allocator_t* allocator =
      (iree_hal_cuda_vma_allocator_t*)base_allocator;
  return allocator->host_allocator;
}

static iree_hal_buffer_compatibility_t
iree_hal_cuda_vma_allocator_query_buffer_compatibility(
...
static iree_status_t iree_hal_cuda_vma_allocator_make_compatible(
...
static iree_status_t iree_hal_cuda_vma_allocator_allocate_internal(
    iree_hal_cuda_vma_allocator_t* allocator,
  ...
  return iree_hal_cuda_vma_buffer_wrap(
...
static iree_status_t iree_hal_cuda_vma_allocator_allocate_buffer(
    iree_hal_allocator_t* base_allocator, iree_hal_memory_type_t memory_type,
  ...
  IREE_RETURN_IF_ERROR(iree_hal_cuda_vma_allocator_make_compatible(
      &memory_type, &allowed_access, &allowed_usage));

  return iree_hal_cuda_vma_allocator_allocate_internal(
      allocator, memory_type, allowed_usage, allowed_access, allocation_size,
      /*flags=*/0, out_buffer);
}

static iree_status_t iree_hal_cuda_vma_allocator_wrap_buffer(
    iree_hal_allocator_t* base_allocator, iree_hal_memory_type_t memory_type,
    iree_hal_memory_access_t allowed_access,
...
const iree_hal_allocator_vtable_t iree_hal_cuda_vma_allocator_vtable = {
    /*.destroy=*/iree_hal_cuda_vma_allocator_destroy,
    /*.host_allocator=*/iree_hal_cuda_vma_allocator_host_allocator,
    /*.query_buffer_compatibility = */
    iree_hal_cuda_vma_allocator_query_buffer_compatibility,
    /*.allocate_buffer=*/iree_hal_cuda_vma_allocator_allocate_buffer,
    /*.wrap_buffer=*/iree_hal_cuda_vma_allocator_wrap_buffer,
};
