...
#include "iree/hal/cuda/descriptor_set_arena.h"
...
static VkPipelineStageFlags iree_hal_cuda_convert_pipeline_stage_flags(
...
static VkAccessFlags iree_hal_cuda_convert_access_mask(
...
static iree_status_t iree_hal_cuda_direct_command_buffer_execution_barrier(
  ...
  for (int i = 0; i < memory_barrier_count; ++i) {
    ...
    info.pNext = NULL;
    info.srcAccessMask =
        iree_hal_cuda_convert_access_mask(memory_barrier.source_scope);
    info.dstAccessMask =
        iree_hal_cuda_convert_access_mask(memory_barrier.target_scope);
  }
  ...
  for (int i = 0; i < buffer_barrier_count; ++i) {
    ...
    info.pNext = NULL;
    info.srcAccessMask =
        iree_hal_cuda_convert_access_mask(buffer_barrier.source_scope);
    info.dstAccessMask =
        iree_hal_cuda_convert_access_mask(buffer_barrier.target_scope);
    info.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    info.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    info.buffer = iree_hal_cuda_vma_buffer_handle(
  ...
  command_buffer->syms->vkCmdPipelineBarrier(
      ...
      iree_hal_cuda_convert_pipeline_stage_flags(source_stage_mask),
      iree_hal_cuda_convert_pipeline_stage_flags(target_stage_mask),
...
static iree_status_t iree_hal_cuda_direct_command_buffer_signal_event(
  ...
  command_buffer->syms->vkCmdSetEvent(
      command_buffer->handle, iree_hal_cuda_native_event_handle(event),
      iree_hal_cuda_convert_pipeline_stage_flags(source_stage_mask));
...
static iree_status_t iree_hal_cuda_direct_command_buffer_wait_events(
  ...
  for (int i = 0; i < event_count; ++i) {
    event_handles[i] = iree_hal_cuda_native_event_handle(events[i]);
  }
...
static iree_status_t iree_hal_cuda_direct_command_buffer_fill_buffer(
  ...
  iree_hal_cuda_direct_command_buffer_t* command_buffer =
      iree_hal_cuda_direct_command_buffer_cast(base_command_buffer);
  VkBuffer target_device_buffer = iree_hal_cuda_vma_buffer_handle(
  ...
  uint32_t dword_pattern =
      iree_hal_cuda_splat_pattern(pattern, pattern_length);
...
static iree_status_t iree_hal_cuda_direct_command_buffer_update_buffer(
  ...
  // Cuda only allows updates of <= 65536 because you really, really, really
...
static iree_status_t iree_hal_cuda_direct_command_buffer_copy_buffer(
  ...
  iree_hal_cuda_direct_command_buffer_t* command_buffer =
      iree_hal_cuda_direct_command_buffer_cast(base_command_buffer);
  VkBuffer source_device_buffer = iree_hal_cuda_vma_buffer_handle(
...
static iree_status_t iree_hal_cuda_direct_command_buffer_push_constants(
  ...
  command_buffer->syms->vkCmdPushConstants(
      command_buffer->handle,
      iree_hal_cuda_native_executable_layout_handle(executable_layout),
...
static iree_status_t iree_hal_cuda_direct_command_buffer_bind_descriptor_set(
  ...
  // Cuda takes uint32_t as the size here, unlike everywhere else.
  ...
  VkDescriptorSet descriptor_sets[1] = {
      iree_hal_cuda_native_descriptor_set_handle(descriptor_set),
  };
  command_buffer->syms->vkCmdBindDescriptorSets(
      command_buffer->handle, VK_PIPELINE_BIND_POINT_COMPUTE,
      iree_hal_cuda_native_executable_layout_handle(executable_layout), set,
...
static iree_status_t iree_hal_cuda_direct_command_buffer_dispatch(
  ...
  IREE_RETURN_IF_ERROR(
      iree_hal_cuda_native_executable_pipeline_for_entry_point(
...
static iree_status_t iree_hal_cuda_direct_command_buffer_dispatch_indirect(
  ...
  VkBuffer workgroups_device_buffer = iree_hal_cuda_vma_buffer_handle(
