...
//
// This originates from Cuda having a lower-bound of INT_MAX for
...
//
// See:
//   https://cuda.gpuinfo.org/displayextensionproperty.php?name=maxTimelineSemaphoreValueDifference
#define IREE_HAL_CUDA_SEMAPHORE_MAX_VALUE (2147483647ull - 1)
...
} iree_hal_cuda_native_semaphore_t;

extern const iree_hal_semaphore_vtable_t
    iree_hal_cuda_native_semaphore_vtable;

static iree_hal_cuda_native_semaphore_t*
iree_hal_cuda_native_semaphore_cast(iree_hal_semaphore_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_native_semaphore_vtable);
  return (iree_hal_cuda_native_semaphore_t*)base_value;
}

iree_status_t iree_hal_cuda_native_semaphore_create(
    iree::hal::cuda::VkDeviceHandle* logical_device, uint64_t initial_value,
  ...
  *out_semaphore = NULL;
  ...
  iree_hal_cuda_native_semaphore_t* semaphore = NULL;
  iree_status_t status = iree_allocator_malloc(
      logical_device->host_allocator(), sizeof(*semaphore), (void**)&semaphore);
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_cuda_native_semaphore_vtable,
                                 &semaphore->resource);
...
static void iree_hal_cuda_native_semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_cuda_native_semaphore_t* semaphore =
      iree_hal_cuda_native_semaphore_cast(base_semaphore);
...
VkSemaphore iree_hal_cuda_native_semaphore_handle(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_cuda_native_semaphore_t* semaphore =
      iree_hal_cuda_native_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_cuda_native_semaphore_query(
  ...
  *out_value = 0;
  ...
  if (value > IREE_HAL_CUDA_SEMAPHORE_MAX_VALUE) {
...
static iree_status_t iree_hal_cuda_native_semaphore_signal(
...
static void iree_hal_cuda_native_semaphore_fail(
  ...
  signal_info.pNext = NULL;
  signal_info.semaphore = semaphore->handle;
  signal_info.value = IREE_HAL_CUDA_SEMAPHORE_MAX_VALUE + 1;
...
iree_status_t iree_hal_cuda_native_semaphore_multi_wait(
  ...
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    semaphore_handles[i] =
        iree_hal_cuda_native_semaphore_handle(semaphore_list->semaphores[i]);
  }
  ...
  static_assert(
      ...
      "payload value type must match cuda expected size");
...
static iree_status_t iree_hal_cuda_native_semaphore_wait_with_deadline(
...
static iree_status_t iree_hal_cuda_native_semaphore_wait_with_timeout(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_duration_t timeout_ns) {
  return iree_hal_cuda_native_semaphore_wait_with_deadline(
      base_semaphore, value, iree_relative_timeout_to_deadline_ns(timeout_ns));
}

const iree_hal_semaphore_vtable_t iree_hal_cuda_native_semaphore_vtable = {
    /*.destroy=*/iree_hal_cuda_native_semaphore_destroy,
    /*.query=*/iree_hal_cuda_native_semaphore_query,
    /*.signal=*/iree_hal_cuda_native_semaphore_signal,
    /*.fail=*/iree_hal_cuda_native_semaphore_fail,
    /*.wait_with_deadline=*/iree_hal_cuda_native_semaphore_wait_with_deadline,
    /*.wait_with_timeout=*/iree_hal_cuda_native_semaphore_wait_with_timeout,
};
