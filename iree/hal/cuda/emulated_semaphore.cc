...
class EmulatedTimelineSemaphore final {
 public:
  EmulatedTimelineSemaphore(VkDeviceHandle* logical_device,
                            ...
                            iree::hal::cuda::CommandQueue** command_queues,
...
// not sure it's worth it (given that we may require Cuda 1.2 with timeline
// semaphores built in at some point soon).
typedef struct {
  iree_hal_resource_t resource;
  iree_allocator_t host_allocator;
  EmulatedTimelineSemaphore* handle;
} iree_hal_cuda_emulated_semaphore_t;

extern const iree_hal_semaphore_vtable_t
    iree_hal_cuda_emulated_semaphore_vtable;

static EmulatedTimelineSemaphore* iree_hal_cuda_emulated_semaphore_cast(
    iree_hal_semaphore_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_emulated_semaphore_vtable);
  return ((iree_hal_cuda_emulated_semaphore_t*)base_value)->handle;
}

iree_status_t iree_hal_cuda_emulated_semaphore_create(
    iree::hal::cuda::VkDeviceHandle* logical_device,
    iree::hal::cuda::TimePointSemaphorePool* semaphore_pool,
    iree_host_size_t command_queue_count,
    iree::hal::cuda::CommandQueue** command_queues, uint64_t initial_value,
    iree_hal_semaphore_t** out_semaphore) {
  iree_hal_cuda_emulated_semaphore_t* semaphore = NULL;
  IREE_RETURN_IF_ERROR(iree_allocator_malloc(logical_device->host_allocator(),
                                             sizeof(*semaphore),
                                             (void**)&semaphore));
  iree_hal_resource_initialize(&iree_hal_cuda_emulated_semaphore_vtable,
                               &semaphore->resource);
...
static void iree_hal_cuda_emulated_semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_cuda_emulated_semaphore_t* semaphore =
      (iree_hal_cuda_emulated_semaphore_t*)base_semaphore;
...
iree_status_t iree_hal_cuda_emulated_semaphore_acquire_wait_handle(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    const iree::ref_ptr<iree::hal::cuda::TimePointFence>& wait_fence,
    VkSemaphore* out_handle) {
  EmulatedTimelineSemaphore* semaphore =
      iree_hal_cuda_emulated_semaphore_cast(base_semaphore);
  *out_handle = semaphore->GetWaitSemaphore(value, wait_fence);
  return iree_ok_status();
}

iree_status_t iree_hal_cuda_emulated_semaphore_cancel_wait_handle(
...
iree_status_t iree_hal_cuda_emulated_semaphore_acquire_signal_handle(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    const iree::ref_ptr<iree::hal::cuda::TimePointFence>& signal_fence,
    VkSemaphore* out_handle) {
  EmulatedTimelineSemaphore* semaphore =
      iree_hal_cuda_emulated_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_cuda_emulated_semaphore_query(
...
static iree_status_t iree_hal_cuda_emulated_semaphore_signal(
...
static void iree_hal_cuda_emulated_semaphore_fail(
...
static iree_status_t iree_hal_cuda_emulated_semaphore_wait_with_deadline(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_time_t deadline_ns) {
  EmulatedTimelineSemaphore* semaphore =
      iree_hal_cuda_emulated_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_cuda_emulated_semaphore_wait_with_timeout(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_duration_t timeout_ns) {
  return iree_hal_cuda_emulated_semaphore_wait_with_deadline(
      base_semaphore, value, iree_relative_timeout_to_deadline_ns(timeout_ns));
}

iree_status_t iree_hal_cuda_emulated_semaphore_multi_wait(
    iree::hal::cuda::VkDeviceHandle* logical_device,
  ...
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    IREE_RETURN_IF_ERROR(iree_hal_cuda_emulated_semaphore_wait_with_deadline(
...
const iree_hal_semaphore_vtable_t iree_hal_cuda_emulated_semaphore_vtable = {
    /*.destroy=*/iree_hal_cuda_emulated_semaphore_destroy,
    /*.query=*/iree_hal_cuda_emulated_semaphore_query,
    /*.signal=*/iree_hal_cuda_emulated_semaphore_signal,
    /*.fail=*/iree_hal_cuda_emulated_semaphore_fail,
    /*.wait_with_deadline=*/
    iree_hal_cuda_emulated_semaphore_wait_with_deadline,
    /*.wait_with_timeout=*/
    iree_hal_cuda_emulated_semaphore_wait_with_timeout,
};
