...
iree_status_t iree_hal_cuda_result_to_status(VkResult result,
                                               const char* file,
                                               uint32_t line) {
  switch (result) {
    // Success codes.
    ...
    case VK_ERROR_INCOMPATIBLE_DRIVER:
      // The requested version of Cuda is not supported by the driver or is
    ...
    case VK_ERROR_NATIVE_WINDOW_IN_USE_KHR:
      // The requested window is already in use by Cuda or another API in a
    ...
    case VK_ERROR_INVALID_DRM_FORMAT_MODIFIER_PLANE_LAYOUT_EXT:
      ...
      // outside the scope of Cuda, and therefore not described by Valid Usage
