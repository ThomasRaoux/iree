...
// We exclusively use Cuda via queried function pointers. To ensure that there
// are no accidental calls to the linker-loaded implicit functions we just
// compile them all out.
//
// Code under iree/hal/cuda/ *MUST NOT* directly include cuda.h or any
// header that includes it without this first being set. This means that this
// iree/hal/cuda/cuda_headers.h file must usually be included first in all
// files using it.
//
// From there, use iree/hal/cuda/dynamic_symbols.h to plumb the dynamically
// resolved symbols to any code that may need to make Cuda calls. See that
...
//
// There's a bunch of reasons to dynamically link against Cuda like supporting
// platforms without Cuda or with differing Cuda versions where all symbols
