...
// some amount of Cuda state on the stack. Assume that creating or deleting
// Cuda objects, issuing most Cuda commands, etc are off-limits.
static VKAPI_ATTR VkBool32 VKAPI_CALL
iree_hal_cuda_debug_utils_message_callback(
...
static void iree_hal_cuda_debug_reporter_populate_create_info(
...
iree_status_t iree_hal_cuda_debug_reporter_allocate(
  ...
  iree_hal_cuda_debug_reporter_populate_create_info(&create_info);
  ...
  if (iree_status_is_ok(status)) {
    *out_reporter = reporter;
    ...
    iree_hal_cuda_debug_reporter_free(reporter);
  }
