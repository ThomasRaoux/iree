...
typedef struct {
  ...
  iree_hal_cuda_instance_extensions_t instance_extensions;
  ...
  // The Cuda instance that all devices created from the driver will share.
  VkInstance instance;
  bool owns_instance;
...
static void iree_hal_cuda_driver_populate_default_app_info(
...
static iree_status_t iree_hal_cuda_driver_create_internal(
  ...
  iree_hal_cuda_instance_extensions_t instance_extensions =
      iree_hal_cuda_populate_enabled_instance_extensions(enabled_extensions);
  ...
  if (instance_extensions.debug_utils) {
    IREE_RETURN_IF_ERROR(iree_hal_cuda_debug_reporter_allocate(
...
static iree_status_t iree_hal_cuda_driver_compute_enabled_extensibility_sets(
  ...
  iree_hal_cuda_string_list_t required_layers;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_driver_query_extensibility_set(
      requested_features,
      IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_LAYERS_REQUIRED, arena,
      &required_layers));
  iree_hal_cuda_string_list_t optional_layers;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_driver_query_extensibility_set(
      requested_features,
      IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_LAYERS_OPTIONAL, arena,
      &optional_layers));
  iree_hal_cuda_string_list_t required_extensions;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_driver_query_extensibility_set(
      requested_features,
      IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_EXTENSIONS_REQUIRED, arena,
      &required_extensions));
  iree_hal_cuda_string_list_t optional_extensions;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_driver_query_extensibility_set(
      requested_features,
      IREE_HAL_CUDA_EXTENSIBILITY_INSTANCE_EXTENSIONS_OPTIONAL, arena,
      &optional_extensions));
  ...
  IREE_RETURN_IF_ERROR(iree_hal_cuda_match_available_instance_layers(
      syms, &required_layers, &optional_layers, arena, out_enabled_layers));
  IREE_RETURN_IF_ERROR(iree_hal_cuda_match_available_instance_extensions(
...
IREE_API_EXPORT iree_status_t IREE_API_CALL iree_hal_cuda_driver_create(
  ...
  iree_hal_cuda_driver_populate_default_app_info(options, &app_info);
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_driver_create_internal(
...
static iree_status_t iree_hal_cuda_driver_enumerate_physical_devices(
    iree::hal::cuda::DynamicSymbols* instance_syms, VkInstance instance,
...
static iree_host_size_t iree_hal_cuda_calculate_device_info_size(
...
static iree_status_t iree_hal_cuda_driver_query_available_devices(
  ...
  // Query all devices from the Cuda instance.
  uint32_t physical_device_count = 0;
  VkPhysicalDevice* physical_devices = NULL;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_driver_enumerate_physical_devices(
  ...
  for (uint32_t i = 0; i < physical_device_count; ++i) {
    total_size += iree_hal_cuda_calculate_device_info_size(
...
static iree_status_t iree_hal_cuda_driver_create_device(
  ...
  // devices. Today it *must* be cuda* to work, whereas really that should be
