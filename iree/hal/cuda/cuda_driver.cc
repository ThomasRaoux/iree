...
typedef char cuda_device_name_t[100];
...
static uint8_t* iree_hal_cuda_populate_device_info(
    CUdevice device, iree::hal::cuda::DynamicSymbols* syms, uint8_t* buffer_ptr,
    iree_hal_device_info_t* out_device_info) {
  cuda_device_name_t device_name;
  CUDA_CHECK_OK(syms,
...
static iree_status_t iree_hal_cuda_driver_query_available_devices(
  ...
  for (int i = 0; i < device_count; ++i) {
    total_size += sizeof(cuda_device_name_t);
  }
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (int i = 0; i < device_count; ++i) {
      CUdevice device;
      CUDA_RETURN_IF_ERROR((&driver->syms), cuDeviceGet(&device, i),
                           "cuDeviceGet");
