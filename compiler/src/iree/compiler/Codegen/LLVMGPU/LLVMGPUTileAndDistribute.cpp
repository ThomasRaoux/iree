...
// Split input/output operand into a separate input.
static void replaceInputOutput(Value source, linalg::GenericOp op) {
...
static bool propagateIntoConsumer(memref::CopyOp copyOp) {
  // Look for a generic Op reading the copyOp source.
  ...
  while (nextOp) {
    ...
    replaceInputOutput(copyOp.getSource(), consumer);
    return true;
  }
  return false;
}
...
static void propagateSharedMemCopy(func::FuncOp funcOp) {
  ...
  funcOp.walk([&toDelete](memref::CopyOp copyOp) {
    if (hasMarker(copyOp, getCopyToWorkgroupMemoryMarker())) {
      if (propagateIntoProducer(copyOp) ||  propagateIntoConsumer(copyOp))
