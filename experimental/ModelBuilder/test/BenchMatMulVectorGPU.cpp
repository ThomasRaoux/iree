...
static llvm::cl::opt<bool> correctness(
    "correctness",
    llvm::cl::desc(
        "Compare the result to value calclated on CPU. We will use a smaller "
...
void matMul(int m, int n, int k, int tileM, int tileN, int tileK,
  ...
  options.loweringPasses = [&](mlir::PassManager &pm) {
    ...
    // The number of workgroups are swapped because PLoopToGPU pattern assigns
    // dimension in reverse order of the loop.
    // TODO(thomasraoux): Figure out the correct way to handle this.
...
int main(int argc, char **argv) {
  ...
  int tileK = 32;
  for (int tileM = 8; tileM <= 128; tileM = tileM * 2) {
    for (int tileN = 8; tileN <= 128; tileN = tileN * 2) {
      int m = 4096;
      int n = 4096;
      int k = 4096;
      if (correctness) {
      ...
      // folding is fixed.
      m = (m + tileM - 1) / tileM * tileM;
      n = (n + tileN - 1) / tileN * tileN;
      k = (k + tileK - 1) / tileK * tileK;
